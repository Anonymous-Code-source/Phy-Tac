# -*- coding: utf-8 -*-
"""
Latent Diffusion for Xsense (Using Pretrained Frozen VAE)
- 从 cfg['vae'] 加载已训练好的 VAE: AutoencoderKL_SD（权重冻结）
- 条件：输入图像zin、触觉深度zcdp、质量/纹理tokens
- v-pred + Min-SNR(γ=2.0) + Huber(δ=0.05)；像素损失(MAE+LPIPS)余弦ramp
- EMA 仅用于验证/采样；DDIM采样末步无残噪；指标在统一域上计算
基于你们原程序结构扩展，兼容 TensorBoard/CSV/可视化网格等。
"""
...
def make_xsense_dataset(meta_path: str, data_cfg: Dict):
    ...
    if img_size is None:
        raise ValueError("data.img_size 未指定。")
...
def main():
    ...
    parser.add_argument("--cfg", type=str, default="ldm_xsense_new_vesion.yaml")
    ...
    while step < max_steps:
        ...
        for batch in itr:
            ...
            if (step % val_every == 0) and (step > 0):
                ...
                # 用联合准则 crit=(100-PSNR)+100*LPIPS 选best（LPIPS缺失时等效0.2）
                lpips_eff = val_lpips if (val_lpips is not None) else 0.2
