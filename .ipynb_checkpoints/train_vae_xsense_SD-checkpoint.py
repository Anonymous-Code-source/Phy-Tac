...
# -*- coding: utf-8 -*-
"""
Stable-Diffusion-style AutoencoderKL (缩小版) for Xsense tactile images
---------------------------------------------------------------------
- 与 SD 相同范式：KL-VAE（连续潜空间），f=8 压缩；中间层含轻量自注意力（可关）。
- 规模下调：可配置 `base_ch`、`ch_mult`、`z_ch`（例如 z_ch=2），更适合小数据集。
- 仅使用你们现有的数据处理：src/xsense_dataset.py / src/mt_embedder_cmd.py。
- 配置文件：vae_sd_xsense.yaml。
- 训练阶段仅展示/记录 loss；验证阶段记录 MAE/RMSE/PSNR/SSIM/LPIPS，并保存样例图与 CSV 指标。
- 数值稳定：fp32 重参数化 + logvar clamp、自动 pad→crop（到 8 的倍数）、梯度裁剪。

【变更要点】
- 新增保存策略：同时维护
  1) 最优（best）轻量权重：仅 model.state_dict()，供 LDM diffusion 直接加载；
  2) 最后一次（last）完整训练态：含优化器/步数等，便于继续训练；
  3) 保留原有 best.pt（完整）与周期性 epochN 快照。
"""
...
CONFIG_PATH = Path("vae_sd_xsense.yaml")
...
from mt_embedder_cmd import MassTextureCmdEmbedder  # noqa: E402  (不直接用，仅确保依赖完整)
...
class AutoencoderKL_SD(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x_pad, _, _ = self._pad_to_multiple(x, 2 ** self.downs)  # 对齐到 8 的倍数
...
def main():
    assert CONFIG_PATH.exists(), f"找不到配置文件: {CONFIG_PATH}"
    try:
        with open(CONFIG_PATH, 'r') as f:
            cfg = yaml.safe_load(f)
    except yaml.YAMLError as e:
        print(f"[ERROR] 配置文件解析失败: {CONFIG_PATH}\n{e}\n\n请确认 YAML 语法（冒号、缩进、不要混入分隔线/注释内容）。")
        raise
    ...
    if _LPIPS_OK:
        ...
        print('[INFO] LPIPS 可用，将在验证阶段记录 lpips 值。')
    else:
        lpips_fn = None
        print('[WARN] 未安装 lpips（pip install lpips），将跳过 LPIPS 指标。')
...
#         if lpips_batches > 0:
#             writer.add_scalar('val/lpips', lpips_sum / lpips_batches, epoch)
        lpips_avg = None
        if lpips_batches > 0:
    ...
    for epoch in range(1, int(cfg['epochs']) + 1):
        ...
        if best_by == 'lpips':
            ...
            if not use_lpips:
                print("[WARN] 配置要求用 LPIPS 选最优，但当前未得到 LPIPS，退回使用 val_loss。")
        elif best_by == 'auto':
            use_lpips = (lpips_avg is not None)
        else:  # 'loss'
            use_lpips = False
        ...
        # 1) 如果更好：更新 best（完整 & 轻量）
#         if val_loss < best_val:
#             best_val = val_loss
#             torch.save(state, ckpt_dir / 'best.pt')  # 完整训练态
#             if export_model_only:
#                 torch.save(model.state_dict(), ckpt_dir / export_best_name)  # 仅模型参数
#             print(f"[INFO] New best @ epoch {epoch}: {best_val:.6f}")
        ...
        # 2) 保存 last（每个 epoch 都覆盖）
        if save_last:
            torch.save(state, ckpt_dir / 'last.pt')  # 完整训练态
            if export_model_only:
                torch.save(model.state_dict(), ckpt_dir / export_last_name)  # 仅模型参数

        # 3) 保留周期性 epochN 完整快照（原逻辑）
