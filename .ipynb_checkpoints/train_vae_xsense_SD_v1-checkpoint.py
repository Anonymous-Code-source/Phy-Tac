...
- 配置文件：configs/vae_sd_xsense.yaml（见文件末尾模板）。
...
def main():
    ...
    for epoch in range(1, int(cfg['epochs']) + 1):
        ...
        state = {'epoch': epoch, 'global_step': global_step, 'model': model.state_dict(),
                 'opt': opt.state_dict(), 'cfg': cfg, 'best_val': best_val}
        if val_loss < best_val:
            best_val = val_loss
            torch.save(state, work_dir / 'ckpt' / 'best.pt')
        if (epoch % int(cfg['save_every'])) == 0:
            torch.save(state, work_dir / 'ckpt' / f'epoch{epoch:03d}.pt')
