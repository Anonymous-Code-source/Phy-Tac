...
"""
这一版本欠缺的是：
    1.最优模型的保存以PSNR和LPIPS联合决定：min{（100-PSNR）+(100*LPISP)}; 
    2.更优的训练loss,以获得更好的训练效果
...
class UNetLDM(nn.Module):
    ...
    def forward(self, z_noisy, z_in, z_cdp, t, cond_tokens):
        ...
        # Up (up -> align -> concat -> blocks/attn)
        for m in self.ups:
            x = m["up"](x)
            skip = skips.pop()
...
# -------------------- EVALUATE (with MAE/RMSE/SSIM/PSNR/LPIPS) --------------------
@torch.no_grad()
...
def gc_ckpts(out_dir: str, keep_n: int, prefix: str = "ckpt_", suffix: str = ".pt"):
    ...
    numbered.sort(key=lambda x: x[0])  # 按 step 排序
    to_delete = [f for _, f in numbered[:-keep_n]]  # 仅删多余的旧文件
    for f in to_delete:
        try:
        ...
        for batch in itr:
            ...
            with autocast_fp16(tr["amp"]):
                ...
                if v_pred:
                    ...
                    loss = F.mse_loss(pred_c, v_target)
                else:
                    loss = F.mse_loss(pred_c, eps)
            ...
            if (step % val_every == 0) and (step > 0):
                ...
                try:
                    ...
                    with open(csv_path, "a", newline="") as f:
                        w = csv.writer(f)
                        if write_header:
                            w.writerow(["step","loss","mae","rmse","psnr","ssim","lpips","vis_path"])
                        w.writerow([
